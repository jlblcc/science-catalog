...
let service = {
    ...
    SIMPLE_VOCABULARIES: { // note onlineResource.uri is not unique, nor is much else about these "thesaurus" entries
        'keywords_project_category': 'lcc_project_category',//'LCC Project Category',
        'keywords_end_user_types': 'lcc_end_user_type',//'LCC End User Type',
        'keywords_deliverable_types': 'lcc_deliverable',//'LCC Deliverable',
        'keywords_us_states': 'us_states',//'U.S. States'
        'keywords_congressional_districts': 'federal_congressional_districts',//'Federal Congressional Districts'
    },
    simpleReport: function(resource,keywordType,queryGenerator) {
        return function(req,res) {
            (queryGenerator ? queryGenerator(req) : Promise.resolve({})).then(query => {
                resource.getModel().distinct(`simplified.keywords.keywords.${keywordType}`,(query||{}),function(err,keywords){
                    if(err){
                        return Resource.sendError(res,500,'error getting keywords',err);
                    }
                    res.send(keywords);
